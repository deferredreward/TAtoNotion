...
def process_nested_blockquotes(lines, start_index):
    ...
    # Check if we need to insert a blank parent blockquote
    # If the first group is level 2+ and there's no level 1 blockquote
    needs_parent = False
    if level_groups and level_groups[0][0] >= 2 and not any(level == 1 for level, _ in level_groups):
        needs_parent = True
        # Insert a blank level 1 blockquote with a single space instead of empty string
        level_groups.insert(0, (1, [" "]))
    
    # Second pass: create blocks for each group
    for idx, (level, group_lines) in enumerate(level_groups):
        # Skip entirely empty groups unless it's our intentionally added blank parent
        if not any(line.strip() for line in group_lines) and not (needs_parent and idx == 0):
            continue
...
def convert_markdown_to_notion_blocks(markdown_content):
    ...
    while i < len(lines):
        ...
        # Check for blockquotes - handle nested quotes
            ...
            # Check if this is a double blockquote without a preceding single blockquote
            is_double_blockquote_orphan = False
            if line.startswith("> >") and (i == 0 or not lines[i-1].strip().startswith(">")):
                is_double_blockquote_orphan = True
                # Insert a blank blockquote with a single space instead of empty string
                blocks.append({
                    "object": "block",
                    "type": "quote",
                    "quote": {
            ...
            if is_double_blockquote_orphan and quote_blocks:
                # The first block in quote_blocks is likely a level 2 block that should be a child
                # of our blank blockquote
                parent_child_relations.append((len(blocks) - 1, quote_blocks[0]))
                # Remove it from the blocks list since it will be added as a child
                quote_blocks = quote_blocks[1:]
